...
class VirtualBatchNorm1d(Module):
    ...
    def forward(self, x, is_reference: bool):
        ...
        if is_reference:
            ...
            self.ref_mean_sq = Variable(mean_sq.data.clone(), requires_grad=True)
            ...
            out = self._normalize(x, mean, mean_sq).detach_()
        else:
            # calculate new mean and mean_sq
            batch_size = x.size(0)
            new_coeff = 1. / (batch_size + 1.)
            old_coeff = 1. - new_coeff
            mean = new_coeff * x_mean + old_coeff * self.ref_mean
            mean_sq = new_coeff * x_mean_sq + old_coeff * self.ref_mean_sq
    ...
    def _normalize(self, x, mean, mean_sq):
        ...
        if self.mean.size(1) != self.num_features:
            raise Exception(
                    'Mean size not equal to number of featuers : given {}, expected {}'
                    .format(self.mean.size(1), self.num_features))
        if self.mean_sq.size(1) != self.num_features:
            raise Exception(
                    'Squared mean tensor size not equal to number of features : given {}, expected {}'
                    .format(self.mean_sq.size(1), self.num_features))
